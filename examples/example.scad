...
box_shell1([50,40,20],wall_bot=1.2,wall_top=1.2,wall_side=1.6,rim_gap=0,rbot=1,rbot_inside=2,rtop=1,rtop_inside=1,rsides=5,base_height=0)
{
    ...
    box_inside()
    {
        ...
        X(15) box_pos(CENTER) box_flip() box_screw_clamp(rounding=0.5);
        ...
        X(-7) { // wall. TODO: make module?
            ...
            w = 1;
            f = 1;
            box_add_base() box_pos(FRONT) diff() cuboid([w,$parent_size.y,$base_height],rounding=-f,edges=edges,anchor=FRONT+BOTTOM) position(CENTER) orient(RIGHT) tag("remove") cyl(h=w*2,d=3);
