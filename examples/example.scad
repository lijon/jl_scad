...
box_shell1([50,40,20],wall_bot=2.2,wall_top=1.2,wall_side=1.6,rim_gap=0,rbot=1,rbot_inside=2,rtop=1,rtop_inside=1,rsides=5,base_height=0,hide=false)
{
    ...
    {
        box_add_base() {
            M(10,25) box_pos() standoff(h=5); // default box anchor is left+front corner.
        ...
        X(17) {
            box_add_base() box_pos(LEFT) box_wall(BACK,width=1,fillet=1.5);
            box_add_lid() box_pos(LEFT) box_wall(BACK,width=1,fillet=1.5);
        }
